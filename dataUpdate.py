...
realm = input("Choose Realm:\n- BR\n- EUNE\n- EUW\n"
              "- JP\n- KR\n- LAN\n- LAS\n- NA\n- OCE"
              "\n- RU\n- TR\n>>>").lower()
api_version_url = 'https://ddragon.leagueoflegends.com/api/versions.json'
realm_version_url = ("https://ddragon.leagueoflegends.com/realms/" + realm + ".json")
cdn_url = 'http://ddragon.leagueoflegends.com/cdn/'

try:
    def LoadJsonFromUrl(url):
        response = requests.get(url)
        responseEnc = response.text
        return json.loads(responseEnc)
    ...
    champs_url = f'{cdn_url}{champs_vers}/data/en_US/championFull.json'
    runes_url = f'{cdn_url}{runes_vers}/data/en_US/rune.json'

    print(f'Realm: {realm.upper()}')
